...
*/
package com.music.audiovisualizer.visualizer;
...
import com.music.audiovisualizer.BaseVisualizer;
import com.music.audiovisualizer.common.AVConstants;
import com.music.audiovisualizer.model.AnimSpeed;
import com.music.audiovisualizer.model.PositionGravity;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isVisualizationEnabled && mRawAudioBytes != null) {
            ...
            if (nBatchCount == 0) {
                ...
                for (int i = 0; i < mSrcY.length; i++) {
                    int x = (int) Math.ceil((i + 1) * (1f * mRawAudioBytes.length / nPoints));
                    int t = 0;
                    if (x < 1024)
                        t = getHeight() + ((byte) (Math.abs(mRawAudioBytes[x]) + 128)) * getHeight() / 128;
            ...
            float maxY = getHeight() / 1.36f;
            for (int i = 0; i < mSrcY.length; i++) {
                float barY = mSrcY[i] + (1f * nBatchCount / mMaxBatchCount * (mDestY[i] - mSrcY[i]));
                float barX = (i * mBarWidth) + (mBarWidth / 2);
                if (Math.abs(getHeight() - Math.abs(barY)) > maxY) {
                    barY = getHeight() - maxY;
                }
                canvas.drawLine(barX, getHeight(), barX, barY, mPaint);
            }
