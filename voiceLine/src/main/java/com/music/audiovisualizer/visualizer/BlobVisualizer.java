...
import com.music.audiovisualizer.common.BezierSpline;
...
public class BlobVisualizer extends BaseVisualizer {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mRadius == -1) {
            mRadius = Math.min(getHeight(), getWidth());
        ...
        if (isVisualizationEnabled && mRawAudioBytes != null) {
            ...
            for (int i = 0; i < nPoints; i++, angle += mAngleOffset) {
                ...
                int t = 0;
                if (x < 1024)
                    t = ((byte) (-Math.abs(mRawAudioBytes[x]) + 128)) * (getHeight() / 4) / 128;
            ...
            if (mPaintStyle == PaintStyle.FILL)
                mBlobPath.lineTo(getWidth() / 2f, getHeight() / 2f);
