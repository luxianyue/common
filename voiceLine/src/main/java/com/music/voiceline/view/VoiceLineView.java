...
public class VoiceLineView extends View {
    ...
    //默认volume,当volume的值低于defVolume的时候，set: volume = defVolume
    private int defVolume = 10;
    ...
    private void initAtts(Context context, AttributeSet attrs) {
        ...
        maxVolume = typedArray.getFloat(R.styleable.voiceView_maxVolume, maxVolume);
        defVolume = typedArray.getInt(R.styleable.voiceView_defVolume, defVolume);
        sensibility = typedArray.getInt(R.styleable.voiceView_sensibility, sensibility);
        if (mode == RECT) {
            rectWidth = typedArray.getDimension(R.styleable.voiceView_rectWidth, rectWidth);
            rectSpace = typedArray.getDimension(R.styleable.voiceView_rectSpace, rectSpace);
            rectInitHeight = typedArray.getDimension(R.styleable.voiceView_rectInitHeight, rectInitHeight);
        } else {
            middleLineColor = typedArray.getColor(R.styleable.voiceView_middleLine, Color.BLACK);
            middleLineHeight = typedArray.getDimension(R.styleable.voiceView_middleLineHeight, middleLineHeight);
            lineSpeed = typedArray.getInt(R.styleable.voiceView_lineSpeed, lineSpeed);
            fineness = typedArray.getInt(R.styleable.voiceView_fineness, fineness);
